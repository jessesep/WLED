...
void serializeConfig(JsonObject root) {
  ...
  if (ethernetType != WLED_ETH_NONE && ethernetType < WLED_NUM_ETH_TYPES) {
    ...
    if (ethernetType == WLED_ETH_W5500_SPI) {
      // W5500 SPI pins: MOSI, MISO, SCK, CS, INT, RST
      pins.add(WLED_ETH_W5500_MOSI);
      pins.add(WLED_ETH_W5500_MISO);
      pins.add(WLED_ETH_W5500_SCK);
      pins.add(WLED_ETH_W5500_CS);
      pins.add(WLED_ETH_W5500_INT);
      pins.add(WLED_ETH_W5500_RST);
    } else
    #endif
    {
      // RMII ethernet pins
